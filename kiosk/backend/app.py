...
load_dotenv()
WEB_PORT = os.getenv("MY_KEY_WEB")



face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')

db_path = os.path.join("/app/data_set/user")
TH_voice_id = "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Speech\Voices\Tokens\TTS_THAI"

UPLOAD_FOLDER = os.path.join("/app/data_set/user")
...
def analyze_face(face_roi, x, y, w, h, img_flipped, saved_faces, db_path):
    try:
        ...
        api_url = f"{WEB_PORT}/insert-face"
        data = {
            "name": name,
            "emotion": emotion,
            "age": age,
...
def video_feed():
    camera = cv2.VideoCapture('http://172.22.80.1:5111/video')
...
def user_images(userid, filename):
    ...
    image_path = os.path.join("/app/data_set/user", str(userid))
    print(image_path)
...
def delete_folder(userId):
    ...
    # base_path = os.path.dirname(os.path.abspath(__file__))
    folder_path = os.path.join("/app/data_set/user")
...
def add_folder():
    ...
    if file and allowed_file(file.filename):
        ...
        # base_path = os.path.dirname(os.path.abspath(__file__))
        user_folder  = os.path.join("/app/data_set/user")
...
def add_image():
    ...
    if file and allowed_file(file.filename):
        ...
        # base_path = os.path.dirname(os.path.abspath(__file__))
        user_folder_path  = '/app/data_set/user'
